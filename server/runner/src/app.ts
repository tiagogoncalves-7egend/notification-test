...
export const dataReceived = (dependencies: Dependencies) =>
    flow(
        ...
        E.mapLeft(
            inputError('Input string is not a valid publish request object')
        ),
        TE.fromEither,
...
const postSlackMember =
    ...
    (notification: Notification) => {
        const message = SlackCompose(notification)
        const postAll = A.map(
            flow(postOptions(message), slackClient.postMessage)
        )
        return pipe(
            channel,
...
const postSlackMembersEmail =
    ...
    (notification: Notification) =>
        pipe(
            ...
            A.uniq(string.Eq),
            A.map(email =>
                postSlackMemberEmail(slackClient)(email)(notification)
            ),
            parallel,
    ...
    pipe(
        ...
        TE.chainW(({ id }) =>
            pipe(
                id,
                O.fromNullable,
                TE.fromOption(emptyConversationIdError(name))
            )
        )
    )
